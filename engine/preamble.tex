...
% ============================================================================
%
% MASTER PARAMETERS (loaded from generated/settings.tex — edit content/contact.yaml):
%   \PageFormat .......... Paper size: "a4" (210×297mm) or "letter" (215.9×279.4mm)
%   \PageMarginMM ........ Page margin on all four sides, in mm (sweet spot value)
% ============================================================================

% --- Page format + margin (from content/contact.yaml via generated/settings.tex)
\input{generated/settings.tex}
...
%     \fontspec loads fonts in TeX points:  1pt = 25.4mm / 72.27
%     (NOT PostScript big-points where 1bp = 25.4mm / 72.)
%     Using the correct TeX-pt conversion keeps the grid cell size
%     exactly equal to the monospace advance width at \GridFontSize.
\newcommand{\PtToMM}{\fpeval{25.4 / 72.27}}% = 0.35146… mm per TeX pt
...
%     than our monospace characters. Setting modules in pt (the native
%     font unit) avoids any mm↔pt conversion rounding, so 1 grid column
%     equals 1 monospace character cell exactly.
\newcommand{\CellWidthPt}{\fpeval{\GridFontSize * \MonoWidthRatio}}%  pt
\newcommand{\CellHeightPt}{\fpeval{\GridFontSize}}%                   pt
\TPHorizModule=\CellWidthPt pt\relax
\TPVertModule=\CellHeightPt pt\relax
...
%
%     These commands ONLY control size — the font family (Iosevka Aile)
%     is set separately by \aile inside the typography presets (§7.2).
%     Changing a size command here does NOT change the font, just how
%     big or small the text renders.
%
%     TO CHANGE SIZE: pick a value from the table below and paste
%     the command between the braces { } on the right.
%
...
%     ────────────────────────────────────────────────────────────────
%      5    \tiny
%      5.5  \fontsize{5.5}{6.6}\selectfont
%      6    \fontsize{6}{7.2}\selectfont
%      6.5  \fontsize{6.5}{7.8}\selectfont
%      7    \scriptsize
%      7.5  \fontsize{7.5}{9}\selectfont
%      8    \footnotesize                          ← body text default
%      8.5  \fontsize{8.5}{10.2}\selectfont
%      9    \small                                 ← heading default
%      9.5  \fontsize{9.5}{11.4}\selectfont
%     10    \normalsize
%     10.5  \fontsize{10.5}{12.6}\selectfont
%     11    \fontsize{11}{13.2}\selectfont
%     11.5  \fontsize{11.5}{13.8}\selectfont
%     12    \large
%     14.4  \Large
%     17.28 \LARGE
%     20.74 \huge
...
%     ────────────────────────────────────────────────────────────────
%
%     EXAMPLES — just changing size (font stays Iosevka Aile):
%       \newcommand{\ContentSizeLeft}{\small}
%                                      ^^^^^^ body text becomes 9 pt
%
%       \newcommand{\ContentSizeLeft}{\fontsize{8.5}{10.2}\selectfont}
%                                      ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
%                                      body text becomes 8.5 pt
%                                      (yes, \selectfont is literal —
%                                       you must include it for the
%                                       \fontsize command to work)
%
%     The {10.2} is the line spacing (1.2× the size). It is overridden
%     by \ContentLeading in this CV, so the exact value doesn't matter
%     — just keep it at roughly 1.2× the font size.
%
%     TO CHANGE FONT FAMILY (advanced): edit the \aile / \etoile /
%     \mono shortcuts above, or change the \setmainfont block.
%
\newcommand{\ContentSizeLeft}{\fontsize{7.5}{9}\selectfont}     % left box body text       (8 pt)
\newcommand{\ContentSizeRight}{\fontsize{7.5}{9}\selectfont}    % right box body text      (8 pt)
\newcommand{\HeadingSize}{\footnotesize\bfseries}               % subsection headings      (9 pt bold)
\newcommand{\SecondarySize}{\fontsize{7.5}{9}\selectfont}       % dates, orgs, locations   (8 pt)
...
% --- THEME: Retrowave Red (dark CRT) ----------------------------------------
%     Dark background with red phosphor accents. 1983 WarGames terminal.
% \definecolor{pal-bg}{HTML}{0C0C0C}
% \definecolor{pal-text}{HTML}{D4D4D4}
% \definecolor{pal-structure}{HTML}{662222}
% \definecolor{pal-depth}{HTML}{AA2020}
% \definecolor{pal-accent}{HTML}{EE3333}
% \definecolor{pal-dot}{HTML}{1A0808}
% \definecolor{pal-heading}{HTML}{FF4444}
% \definecolor{pal-subtle}{HTML}{888888}

% --- THEME: Crimson ----------------------------------------------------------
%     Light cream background with warm red structural lines and accents.
% \definecolor{pal-bg}{HTML}{FFFAF8}
% \definecolor{pal-text}{HTML}{2D2020}
% \definecolor{pal-structure}{HTML}{C09090}
% \definecolor{pal-depth}{HTML}{993333}
% \definecolor{pal-accent}{HTML}{CC2200}
% \definecolor{pal-dot}{HTML}{E0D0D0}
% \definecolor{pal-heading}{HTML}{2D2020}
% \definecolor{pal-subtle}{HTML}{7A5A5A}
...
\colorlet{header-shade}{role-accent}         % ASCII-art name glyphs (mainframe/crt themes)
...
\newcommand{\HSHADE}{\color{header-shade}} % ASCII-art name glyphs (mainframe/crt)
...
%   \HeaderContactSep ..... Preferred max dot-spaces between contact items
%                           (auto-reduced by header.tex if content is too wide)
...
% --- Header (master) -------------------------------------------------------
%   Block/shade chars in Iosevka Extended are 11.25pt tall (1.25× the 9pt
%   grid cell).  The CRT and mainframe headers let those rows use their
%   natural height via \FullHRow, so the header occupies more grid rows
%   than the classic theme.
%
%     classic    : 6 rows × 9pt                      = 54pt   → 6 grid rows
%     crt        : 4 × 11.25pt + 2 × 9pt             = 63pt   → 7 grid rows
%     mainframe  : 5 × 11.25pt + 1 × 9pt             = 65.25pt → 7 grid rows
%
\def\tmpThemeClassic{classic}
\def\tmpThemeCrt{crt}
\def\tmpThemeMainframe{mainframe}
\ifx\HeaderTheme\tmpThemeMainframe
    \newcommand{\HeaderHeight}{7}
\else\ifx\HeaderTheme\tmpThemeCrt
    \newcommand{\HeaderHeight}{7}
\else
    \newcommand{\HeaderHeight}{6}
\fi\fi
\newcommand{\HeaderContactSep}{3}           % preferred max dot-spaces between contact items
                                             % (header.tex auto-reduces to fit; set higher for wider margins)
...
% --- Page-break split threshold (master) -----------------------------------
%     When a box overflows the page, the layout engine tries to split it.
%     \MinSplitContentRows is the minimum number of content rows that must
%     fit on the CURRENT page for a split to happen. If fewer rows fit,
%     the entire box is pushed to the next page instead.
%
%     Lower value  → more aggressive splitting (even a small stub stays
%                    on page 1, rest continues on page 2).
%     Higher value → less aggressive (box moves to page 2 whole unless
%                    a meaningful chunk fits on page 1).
%
%     Sensible range: 1–10.  Default: 3.
\newcommand{\MinSplitContentRows}{3}
...
% \FullHRow{content} — stack a header row at its NATURAL glyph height.
%   NO CLIPPING.
%
%   MEASURED FACTS (Iosevka Extended at \GridFontSize = 9pt):
%     All characters: wd = 5.4pt (grid-perfect)
%     Block / shade / vert-spanning chars (█░▓║│▀▄):
%       ht = 8.685pt, dp = 2.565pt, total = 11.25pt
%     Grid cell (\TPVertModule) = 9pt
%     Font leading = 1250/1000 = 1.25  →  9pt × 1.25 = 11.25pt
%
%   Block/shade chars are designed to span the full line height (1.25×em),
%   NOT just the em-square.  Instead of clipping them to fit a 9pt cell
%   (which cuts off ink or creates gaps), we let each row be its natural
%   height.  We RAISE the content by its depth so all ink sits above the
%   baseline (dp→0, ht→ht+dp).  Rows stack flush via \offinterlineskip
%   and the shade/block patterns tile seamlessly with zero gaps.
\newsavebox{\hrowbox}
\newcommand{\FullHRow}[1]{%
    \sbox{\hrowbox}{#1}%
    \dimen0=\dp\hrowbox\relax
    \setbox\hrowbox=\hbox{\raise\dimen0\box\hrowbox}%
    \dp\hrowbox=0pt\relax
    \box\hrowbox\relax
}
...
% ============================================================================
% §8  BOX HEIGHT LOGGING (for layout.py auto page-break engine)
% ============================================================================
% Writes measured content heights to boxheights.dat so scripts/layout.py
% can compute page breaks deterministically. Each box template calls
% \LogBoxHeight{filename}{contentRows} after measuring.
%
% With latexmk -outdir=build, \openout writes relative to the output
% directory, so boxheights.dat lands in build/boxheights.dat automatically.
% ============================================================================
\newwrite\boxheightfile
\immediate\openout\boxheightfile=boxheights.dat\relax

\newcommand{\LogBoxHeight}[2]{%
    % #1 = content file path (e.g. generated/summary.tex)
    % #2 = contentRows (integer)
    \immediate\write\boxheightfile{#1=#2}%
}

\AtEndDocument{\immediate\closeout\boxheightfile}
...
% ============================================================================
% §9  CONTACT DATA — shared macros populated by generated/contact.tex
% ============================================================================
